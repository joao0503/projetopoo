...
public class ServicoDAO extends BaseDAOImpl<ServicoVO> {
	 ...
	 * 
	 * LocalDate dataAtual = LocalDate.now(); DateTimeFormatter formatador =
	 * DateTimeFormatter.ofPattern("yyyy/MM/dd"); String dataAtualString =
	...
	//
	/*
	 * public List<Servico> buscarPorNome(String nome){ Connection con =
	 * getConnection(); String sql = "SELECT * FROM tb_servico WHERE nome = ?"; try
	 * { PreparedStatement ps = con.prepareStatement(sql); ps.setLong(1,
	 * entity.getId()); ResultSet rs = ps.executeQuery(); if (rs.next()) { Orcamento
	 * orc = new Orcamento(); orc.setPeca(rs.getString("peça"));
	 * orc.setServico(rs.getString("serviço")); return orc; } } catch (SQLException
	 * e) { e.printStackTrace(); } finally { closeConnection(); } return null; }
	 */
	// }
}